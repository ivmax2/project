<?php
    ...
    $sql = "SELECT * FROM commodity WHERE commodity_name LIKE '%$sss%'";
    $result = mysqli_query($conn,$sql) or die("Error");
    $row = mysqli_fetch_array($result);
    $data_nums = mysqli_num_rows($result);  
    if(empty($row)){
        echo "<script>alert('查無資料');history.back(-1);</script>";
    }
    else{
        $per = 5; //每頁顯示項目數量
...
<?php
    ...
    <div>
        <div style="text-align:center; color:#696969;">
            ...
            echo "<br /><a href=?page=1&search=$sss>首頁</a> ";
            echo "第 ";
            for( $i=1 ; $i<=$pages ; $i++ ) {
                if( $page-3 < $i && $i < $page+3 ) {
                    echo "<a href=?page=".$i."&search=$sss>".$i."</a> ";
                }
            } 
            echo " 頁 <a href=?page=".$pages."&search=$sss>末頁</a><br/><br/>";
            ?>
        </div>
    </div>
</body>
